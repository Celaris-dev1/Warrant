...
| 2b | *(documented limitation)* | A traversal string whose literal bytes still share the granted prefix (e.g. `repo/acme/docs/../../secrets/key`'s bytes start with `repo/acme/docs/`), or one carrying an embedded NUL | **Allowed by Warrant.** Warrant does not parse filesystem, URL or C-string semantics into an opaque resource string — it cannot know whether a given tool's "resource" is a path, a URL or a database key. **Mitigation (integration responsibility):** canonicalize any resource string derived from untrusted input with `path.Clean` (or the equivalent for your resource's real syntax) and reject the result if it starts with `..`, is absolute when it shouldn't be, or contains a NUL byte, *before* constructing the `token.Call`/`MCPToolCall`/`A2ATask` Warrant sees. The subtest demonstrates both halves: the raw string is allowed, and the canonicalized-then-checked escape attempt is denied. |
...
## Why the two "documented limitation" rows exist
